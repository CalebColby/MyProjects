...
namespace UnitTestProject1
{
	...
	public class SortingUnitTests
	{
        ...
        {
			int[] myArray = new int[] { 40, 10, 32, 5, 35, 57, 23, 51, 29 };

			int pivotindex = Sorter<int>.Partition(myArray, 0, myArray.Length - 1);

			Assert.AreEqual(6, pivotindex);
        }
        ...
        {
			int[] original = { 7, 3, 2, 7, 12, 9 };
			int[] left = { 2, 3, 7 };
			int[] right = { 7, 9, 12 };
			Sorter<int>.Merge(original, left, right);

			string originalString = ArrayToString(original);
			string expectedString2 = ArrayToString(new int[] { 2, 3, 7, 7, 9, 12});

			Assert.AreEqual(expectedString2, originalString);
        }
	}
}
